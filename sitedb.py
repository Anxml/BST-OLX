...
class users():
    ...
    def import_usrdata(self):
        ...
        for usrdta in oneusrdta:
            ...
            users.users_details[eachdatafield[0]] = [
                eachdatafield[1], eachdatafield[2]]
        usrdata_file.close()
...
class list_mgr():
    ...
    def import_list_users(self):
        ...
        for ldata in list_mgr.lists_data:
            list_mgr.user_lists[list_mgr.lists_data[ldata]
                                [6]].append(list_mgr.lists_data[ldata][5])

    def save_list(self, lname, lprice, lfeature, lspecs, laddr, lno, lauth):
        usrdata_file = open('sdb/ldata.json', 'a', encoding='utf-16-le')
        usrdata_file.write('Ⅱ%sⅠ%sⅠ%sⅠ%sⅠ%sⅠ%sⅠ%s' % (
            lname, lprice, lfeature, lspecs, laddr, lno, lauth))
        usrdata_file.close
    ...
    def import_list(self):
        ...
        for eachitem in onelistdata:
            ...
            list_mgr.lists_data[eachdatafield[5]] = [eachdatafield[0], eachdatafield[1],
                                                     eachdatafield[2], eachdatafield[3], eachdatafield[4], eachdatafield[5], eachdatafield[6]]
        list_data.close()
#
# Chat Functionality
#
...
class chats():
    ...
    def newchatstartfunc(chatsender, chatreciever, lno):
        ...
        chats.chats_list[newchatno] = [(chatsender, chatreciever), [(
            'System', '%s contacted %s for %s' % (chatsender, chatreciever, contactlist))]]
